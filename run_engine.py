...
class Engine:
    ...
    def get_markets(self, poly_category, kalshi_category, kalshi_tags):
        ...
        if kalshi_tags is None:
            kalshi_series.extend(self.kalshi_extractor.get_series(category = kalshi_category, tag = None))
        else:
            for tag in kalshi_tags:
                kalshi_series.extend(self.kalshi_extractor.get_series(category = kalshi_category, tag = tag))
        ...
        for series in kalshi_series:
            ...
            for market in markets:
                self.kalshi_markets.append(f"{market['title']} {market['yes_sub_title']}")
    ...
    def get_matching_markets(self):
        matching_pairs = get_matching_pairs(poly_titles_in = self.poly_markets,kalshi_titles_in = self.kalshi_markets)

        self.pair_list = [] 
        for matching_pair in matching_pairs:
            poly_title = matching_pair['poly_title']
            poly_market = self.poly_extractor.title_to_markets.get(poly_title, {})
            kalshi_title = matching_pair['kalshi_title']
            kalshi_market = self.kalshi_extractor.title_to_markets.get(kalshi_title, {})

            p_yes, p_no, p_link = self.poly_extractor.get_market_yn_link(poly_market)
            k_yes, k_no, k_link = self.kalshi_extractor.get_market_yn_link(kalshi_market)

            if None in (kalshi_title, k_yes, k_no, k_link, poly_title, p_yes, p_no, p_link):    
                continue
            self.pair_list.append(ArbitragePair(kalshi_title, k_yes, k_no, k_link, poly_title, p_yes, p_no, p_link))
        
        return self.pair_list
    
    def get_arb_pair_list(self):
        # get all arb pairs and sort by the edge
        self.arbitrage_pair_list = [pair for pair in self.pair_list if pair.arbitrage != "none"]
    ...
    def get_categories_from_file(self, category_name):
        ...
        poly_category = categories[category_name]['poly_category']
...
if __name__ == "__main__":
    ...
    arb_engine = Engine()
    poly_category, kalshi_category, kalshi_tags = arb_engine.get_categories_from_file(category_name)
